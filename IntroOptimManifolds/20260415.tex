...
\bookBox{About the textbook}{\\
    In this material, we will write content written in the textbook in \textcolor{cA}{\textbf{blue regions}}.
    \begin{figure}[H]
        \begin{minipage}{0.5\columnwidth}
            \centering
            \includegraphics[width=0.9\columnwidth]{lectures.png}
        \end{minipage}%
        \begin{minipage}{0.5\columnwidth}
            \centering
            \includegraphics[width=0.9\columnwidth]{software.png}
        \end{minipage}
    \end{figure}
    There are lectures and software (Manopt) related to the textbook.
}
\href{https://mediaspace.epfl.ch/channel/MATH-512+Optimization+on+manifolds/31836}{EPFL (Video Lectures)}
\href{https://www.nicolasboumal.net/book/#lectures}{Lectures} \href{https://www.manopt.org/}{Manopt}
\href{https://www.manopt.org/manifolds.html}{Manopt: Manifolds}

\myBox{My recommendation for learning optimization on manifolds}{\\
    In this material, we will write supplemental content that is not written in the textbook in \textcolor{cC}{\textbf{green regions}}.
    I personally recommend the following books and a website:
    \begin{enumerate}[label=\textbullet, itemsep=0em]
        \item 多様体上の最適化理論, 佐藤寛之, オーム社 (2024)
        \item 多様体の基礎, 松本幸夫, 東京大学出版 (1988) p.107--116: 逆関数の定理の証明
        \item 佐藤一宏先生のブログ
    \end{enumerate}
}
\href{https://ogyahogya.hatenablog.com/entry/2015/01/31/%E3%83%AA%E3%83%BC%E3%83%9E%E3%83%B3%E5%A4%9A%E6%A7%98%E4%BD%93}{Associate Professor Sato Kazuhiro's blog: Riemannian Manifolds}
...
\bookBox{Unit sphere}{
    ...
    \end{equation}
    \textbf{Personal Opinion:} The sphere is actually quite a good example of a manifold, but I recommend NOT relying too much on it for intuition.
}

\bookBox{differential on linear spaces}{
    \setEqN{19}
    \begin{gather}
        (\mathcal{E}, \mathcal{F} \text{ are linear spaces}) \notag \\
        (U \subseteq \mathcal{E}, \quad V \subseteq \mathcal{F} \text{ are open}) \notag \\
        (F \colon U \to V \text{ is smooth}) \notag \\
        \mathrm{D} F(x) \colon \mathcal{E} \to \mathcal{F} \text{ defined by} \notag \\
        \mathrm{D} F(x) [v] = \dv{t} F(x + t v) \Big|_{t=0} = \lim_{t \to 0} \frac{F(x + t v) - F(x)}{t}.
    \end{gather}
}
...
\myBox{Smoothness}{
    ...
    \end{gather*}
    It is known that smoothness of $C^k$ function $f$ is equivalent to the existence and continuity of the $k$-th Fr\'echet derivatives of $f$. The definition of Fr\'echet derivative is as follows:
    \begin{gather*}
        (U \subseteq \mathbb{R}^d \text{ is open}, \quad f\colon U \to \mathbb{R})\\
        \forall y \in U, \quad
        \exists A \colon U \to \mathbb{R}^d \text{ is bounded linear operator}, \\
        \lim_{\norm{h} \to 0} \frac{\norm{f(y+h) - f(y) - A(h)}}{\norm{h}} = 0.
    \end{gather*}
...
\href{https://ogyahogya.hatenablog.com/entry/2018/08/29/%E3%82%AC%E3%83%88%E3%83%BC%E5%BE%AE%E5%88%86%E3%81%A8%E3%83%95%E3%83%AC%E3%83%83%E3%82%B7%E3%82%A7%E5%BE%AE%E5%88%86%EF%BC%9A%E6%96%B9%E5%90%91%E5%BE%AE%E5%88%86%E3%81%A8%E5%8B%BE%E9%85%8D%E3%81%AE}{ガトー微分とフレッシェ微分}
...
\myBox{Notation of $U_x$}{
    In this textbook, unlike in the general definition of a neighborhood, a neighborhood $U$ of $x$ is defined as an open set.
    To explicitly indicate that it is an open neighborhood of $x$, we use the notation ``$\exists U_x$ is open'' instead of simply writing ``$U$.''
}

\myBox{Example: $S^2$ in $\mathbb{R}^3$ (provide more examples in the whiteboard)}{
    \begin{equation*}
...
\myBox{Intuition of Def 3.10}{
    \begin{gather*}
        \mathrm{D} h_x(x) \simeq \mqty(\pdv{h_{x,1}}{x_1} & \cdots & \pdv{h_{x,1}}{x_d} \\ \vdots & \ddots & \vdots \\ \pdv{h_{x,k}}{x_1} & \cdots & \pdv{h_{x,k}}{x_d}) \in \mathbb{R}^{k \times d},\\
        \mathrm{rank}(\mathrm{D} h_x(x)) = k \iff \dim(\ker(\mathrm{D} h_x(x))) = d-k = n.
    \end{gather*}
    Roughly speaking, we have an $n$-dimensional local space along which $h_x$ is constant and hence equal to $0$, since $h_x(x) = 0$. By the definition of the local defining function $h_x$, this $n$-dimensional space is exactly $\mathcal{M} \cap U_x$. Thus, $\mathcal{M}$ is locally like an $n$-dimensional space, which motivates the tangent space $\mathrm{T}_x \mathcal{M}$ and the statement that the dimension of $\mathcal{M}$ is $n$.

    \rule{0.98\textwidth}{0.4pt}\vspace{0.3em}

    Here is another intuition for the definition. It may seem that the cases $n=d$ and $n=d-k$ are defined differently, but they are actually the same.
    \begin{gather*}
...
\bookBox{Def 3.11}{
    The textbook's definition is somewhat simpler than the general one:
    \begin{gather*}
...
\bookBox{Theorem 3.12}{
    \begin{gather*}
        ...
        F(\mathcal{M} \cap U_x) = (\mathbb{R}^{n} \times \{0_k\}) \cap V.
    \end{gather*}
}
...
\myBox{Rewrite of Theorem 3.12}{
    \begin{gather*}
        ...
        F^{-1}((\mathbb{R}^{n} \times \{0_k\}) \cap V) = \mathcal{M} \cap U_x.
    \end{gather*}
}
...
\bookBox{Non-degenerate $\mathrm{D}F^{-1}$}{
    \setEqN{20}
    \begin{equation}
        (\mathrm{D} F(x))^{-1} = \mathrm{D} F^{-1}(F(x)).
    \end{equation}
...
\myBox{Tangent space is a subset of the linear space}{
    ...
    \end{equation*}
    This may look obvious, but it is not completely trivial.
    \begin{equation*}
...
\bookBox{Theorem 3.15}{
    ...
    \vskip\baselineskip
    \textbf{Case $n=d$:} It suffices to show that
    \begin{equation*}
    ...
    Recall Definition 3.10. We have
    \begin{equation*}
        ...
        c(0) = x, \quad c'(0) = v.
    \end{equation*}
    By the definition of the local defining function $h_x$, we have
    \begin{equation*}
        c(t) \in \mathcal{M} \implies h_x(c(t)) = 0_k.
    \end{equation*}
    Thus, we have
    \begin{align*}
                    & 0_k = \dv{t} h_x(c(t)) = \dv{t} h_x(c(t)+t c'(t)) \Big|_{t=0} = \mathrm{D} h_x(c(t)) [c'(t)] \\
        \implies {} & \mathrm{D} h_x(x) [v] = 0                                                                    \\
        \implies {} & v \in \ker(\mathrm{D} h_x(x)).
    \end{align*}
    \textbf{Step2:} $\mathrm{T}_x \mathcal{M} \supseteq (\text{$n$-dimensional subspace of } \mathcal{E})$. Recall Theorem 3.12.
    \begin{gather*}
        \exists U_x \subseteq \mathcal{E} \text{ is open}, \quad
        \exists V \subseteq \mathbb{R}^d \text{ is open}, \\
        \exists F^{-1}\colon V \to U_x \text{ is a diffeomorphism}, \\
    ...
    \begin{gather*}
        ...
        \exists \epsilon > 0, \quad
        c\colon (-\epsilon, \epsilon) \to \mathbb{R}^d, \quad c(t) = F^{-1}(F(x) + t (u, 0_k)) \text{ is smooth},\\
        c(0) = F^{-1}(F(x)) = x, \quad c'(0) =
        \mathrm{D} F^{-1}(F(x)) [(u, 0_k)].
    \end{gather*}
...
\bookBox{Definition 3.16}{
    ...
    (one of the most important points in this material).
}
...
\myBox{The general definition of manifold}{\\
    ...
    \begin{gather*}
        ...
        (U, \phi) \text{ is an $m$-dim.\ coordinate neighborhood} \iff \phi\colon U \to U' \text{ is a homeomorphism}.
    \end{gather*}
    \textbf{Manifold:}
    \begin{gather*}
...
\bookBox{Example 3.19}{
    ...
    It is neither open (obvious) nor closed ($0$ in the complement of the set, which cannot be an interior point) in $\mathbb{R}^3$, and it is not connected (the set consists of two connected components). Still, it is a smooth embedded submanifold of $\mathbb{R}^3$ of dimension $n=2$.
}
...
\myBox{Proposition 3.20 and Exercise 3.25}{
    ...
    \begin{gather*}
        ...
        \exists h_{x,x'}\colon U_{x,x'} \to \mathbb{R}^{k+k'} \text{ is smooth and defined by } h_{x,x'}(y, y') = (h_x(y), h_{x'}(y')),\\
...
\myBox{Lemma for Exercises 3.27--3.29}{
    ...
    \end{equation*}
    These exercises ask us to prove that the given manifolds are not embedded submanifolds of $\mathcal{E}$.
    We can check the conditions in Def 3.10.
    Note that $\mathcal{E} = \mathbb{R}^2$ with $d=2$.

    To solve these exercises, we first prove that if a manifold $\mathcal{M} \in \{\mathcal{X}, \mathcal{C}, \mathcal{P}\}$ is an embedded submanifold of $\mathbb{R}^2$, then the corresponding codimension $k$ must be 1.
    \vskip\baselineskip
...
\myBox{Exercise 3.28}{
    ...
    \end{equation*}
    On $\mathcal{C}$, we can only approach the origin along the positive $x_2$-axis. Thus, $c'(0) = 0$; otherwise, $c$ cannot be smooth (this can be shown by contradiction).
...
\myBox{Exercise 3.29}{
    ...
    \begin{gather*}
        ...
        F^{-1}((\mathbb{R}^{1} \times \{0_1\}) \cap V) = \mathcal{P} \cap U_0. \quad (\bigstar)
    \end{gather*}
    Since $0 \in \mathcal{P} \cap U_0$ by definition, we have $F(0) \in (\mathbb{R}^{1} \times \{0_1\}) \cap V$ by $(\bigstar)$.
    ...
    \end{equation*}
    Since $F^{-1}$ is a diffeomorphism, $c$ is also a diffeomorphism, i.e., bijective, invertible, and both $c$ and $c^{-1}$ are smooth.
    It is obvious that $\{(a+t, 0) : t \in (-\epsilon, \epsilon)\} \subseteq (\mathbb{R}^{1} \times \{0_1\}) \cap V$. Thus, $\{c(t) : t \in (-\epsilon, \epsilon)\} \subseteq \mathcal{P} \cap U_0$ by $(\bigstar)$. Since $c$ is continuous and $c(0)= F^{-1}(a, 0) = F^{-1}(F(0)) = 0$, its image contains points of $\mathcal{P}$ arbitrarily close to $0$. Thus, there exist $\delta_1, \delta_2, \delta_3, \delta_4 > 0$ such that
    \begin{align*}
    ...
    \end{align*}
    Indeed, since $c$ is invertible and smooth (i.e., continuous), these sets are intervals in $(-\epsilon, \epsilon)$. They are also distinct, since $c^{-1}$ and $F$ are bijective and the domains above are four distinct sets.
    We also have that a sequence in $\{ c^{-1}(F((+x^2, +x))) : x \in (0, \delta_1) \}$ converges to $0$ as $x \to 0$, and the same holds for the other three sets. Thus, we obtain four distinct intervals in $(-\epsilon, \epsilon)$ that approach $0$. This is a contradiction, since there can be at most two such intervals in $(-\epsilon, \epsilon)$ that approach $0$. Thus, $\mathcal{P}$ is not a smooth embedded submanifold of $\mathbb{R}^2$.
}
...
\myBox{Proof of Proposition 3.31}{\\
    ...
    \end{figure}
    \begin{gather*}
        \exists \{ U_{x_i} \}_{i \in I} \text{ is a finite open cover of } \mathcal{M},\\
...
\myBox{Exercise 3.33}{
    \begin{equation*}
        \mathcal{M} = \mathrm{Sym}(2)_1 \subseteq \mathcal{E} = \mathrm{Sym}(2).
    \end{equation*}
    ...
    \begin{equation*}
        ...
        \end{cases}
    \end{equation*}
    Then, a smooth extension of $f$ from $U$ to $\mathcal{E}$ does not exist, since it cannot be smooth at $0$.
}

\myBox{Exercise 3.37}{\\
    We add the hypothesis that $\mathcal{M}$ is a smooth embedded submanifold of $\mathcal{E}$. Otherwise, the statement may be undefined in the context of this textbook.
...
\myBox{Exercise 3.40}{\\
    Almost the same as Exercise 3.37. We only show the case $\mathcal{M}=\mathbb{R}^{m_1}, \mathcal{M}'=\mathbb{R}^{m_2}, \mathcal{N}=\mathbb{R}^{n_1+n_2}$.
    Take an arbitrary point $(x, y) \in \mathcal{M} \times \mathcal{M}'$, and define
    \begin{equation*}
        F_1^{(x,y)}\colon \mathbb{R}^{m_1} \to \mathbb{R}^{n_1}, F_1(x') = F(x', y), \quad
        F_2^{(x,y)}\colon \mathbb{R}^{m_2} \to \mathbb{R}^{n_2}, F_2(y') = F(x, y').
    \end{equation*}
    Then, we have
    \begin{equation*}
...
\myBox{Exercise 3.41}{\\
    \textbf{Assumptions:}
    \begin{gather*}
        \mathcal{M} \subseteq \mathcal{E} \text{ is a $d$-dimensional smooth embedded submanifold},\\
        \ell \geq 1, \quad g \colon \mathcal{M} \to \mathbb{R}^{\ell} \text{ is smooth},\quad \mathrm{rank}(\mathrm{D} g(x)) = \ell \text{ for all } x \in g^{-1}(0),\\
        \mathcal{N} = g^{-1}(0) \subseteq \mathcal{M}.
    \end{gather*}
    \textbf{Goal:}
    \begin{gather*}
        \mathcal{N} \text{ is a smooth embedded submanifold of } \mathcal{E},\\
        \dim \mathcal{N} = \dim \mathcal{M} - \ell,\\
        (\mathrm{T}_x \mathcal{N} = \ker(\mathrm{D} g(x)) \subset \mathrm{T}_x \mathcal{M}. \text{   We omit this goal here.})
    \end{gather*}
    \vskip\baselineskip
    By Definition 3.10, we can rewrite the assumptions and the goal as follows.\\
    \textbf{Assumptions(rw):}
    \begin{gather*}
        \forall x \in \mathcal{M}, \quad
        \exists U_x \subseteq \mathcal{E} \text{ is open}, \quad
        \exists h_x\colon U_x \to \mathbb{R}^{d - \dim \mathcal{M}} \text{ is smooth},\\
        (\forall y \in U_x, \quad h_x(y) = 0_{d - \dim \mathcal{M}} \iff y \in \mathcal{M}) \text{ and } (\mathrm{rank}(\mathrm{D} h_x(x)) = d - \dim \mathcal{M}),\\
        \ell \geq 1, \quad g \colon \mathcal{M} \to \mathbb{R}^{\ell} \text{ is smooth},\quad \mathrm{rank}(\mathrm{D} g(x)) = \ell \text{ for all } x \in g^{-1}(0),\\
        \mathcal{N} = g^{-1}(0) \subseteq \mathcal{M}.
    \end{gather*}
    \textbf{Goal(rw):}
    \begin{gather*}
        ...
        \exists U_x \subseteq \mathcal{E} \text{ is open}, \quad
        \exists h_x\colon U_x \to \mathbb{R}^{d - \dim \mathcal{M} + \ell} \text{ is smooth},\\
        (\forall y \in U_x, \quad h_x(y) = 0_{d - \dim \mathcal{M} + \ell} \iff y \in \mathcal{N}) \text{ and } (\mathrm{rank}(\mathrm{D} h_x(x)) = d - \dim \mathcal{M} + \ell).
    \end{gather*}
    Then, we can construct a local defining function for $\mathcal{N}$ at $x$ as follows:
    \begin{gather*}
        h'_x\colon U_x \to \mathbb{R}^{d - \dim \mathcal{M} + \ell}, \quad h'_x(y) = \mqty[h_x(y) \\ g(y)],\\
        \forall y \in U_x, \quad h'_x(y) = 0_{d - \dim \mathcal{M} + \ell} \iff h_x(y) = 0_{d - \dim \mathcal{M}} \text{ and } g(y) = 0_\ell \iff y \in \mathcal{N},\\
        \mathrm{rank}(\mathrm{D} h'_x(x)) = \mathrm{rank}\left(\mqty[\mathrm{D} h_x(x) \\ \mathrm{D} g(x)]\right) = d - \dim \mathcal{M} + \ell \text{ for all } x \in \mathcal{N}.
    \end{gather*}
    This $h'_x$ is exactly the local defining function for $\mathcal{N}$ at $x$, completing the proof.
}
